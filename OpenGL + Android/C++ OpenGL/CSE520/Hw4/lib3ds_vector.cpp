...
//lib3ds_vector.c modified by Tong and renamed to "lib3ds_vector.cpp", 2008 Fall.
...
void 
vector_make(float c[3], float x, float y, float z) {
    c[0] = x;
    c[1] = y;
    c[2] = z;
}
...
void
vector_copy(float dst[3], float src[3]) {
    int i;
    for (i = 0; i < 3; ++i) {
        dst[i] = src[i];
    }
 ...
 */
void
vector_add(float c[3], float a[3], float b[3]) {
    int i;
    for (i = 0; i < 3; ++i) {
        c[i] = a[i] + b[i];
    }
 ...
 */
void
vector_sub(float c[3], float a[3], float b[3]) {
    int i;
    for (i = 0; i < 3; ++i) {
        c[i] = a[i] - b[i];
    }
 ...
 */
void
vector_scalar_mul(float c[3], float a[3], float k) {
    int i;
    for (i = 0; i < 3; ++i) {
        c[i] = a[i] * k;
    }
 ...
 */
void
vector_cross(float c[3], float a[3], float b[3]) {
    c[0] = a[1] * b[2] - a[2] * b[1];
    c[1] = a[2] * b[0] - a[0] * b[2];
    c[2] = a[0] * b[1] - a[1] * b[0];
}
 ...
 */
float
vector_dot(float a[3], float b[3]) {
 ...
 */
void
vector_normal(float n[3], float a[3], float b[3], float c[3]) {
    float p[3], q[3];
 ...
 */
void
vector_transform(float c[3], float m[4][4], float a[3]) {
 ...
 */
void
vector_min(float c[3], float a[3]) {
    int i;
    for (i = 0; i < 3; ++i) {
        if (a[i] < c[i]) {
            c[i] = a[i];
 ...
 */
void
vector_max(float c[3], float a[3]) {
    int i;
    for (i = 0; i < 3; ++i) {
        if (a[i] > c[i]) {
            c[i] = a[i];
