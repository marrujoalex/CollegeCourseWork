/*
  ...
  Shader sources are in files "bounce.vert" and "tests.frag".
  @Author: T.L. Yu, 2009
*/
#include <stdlib.h>
#include <stdio.h>
...
int init(void)
{
   ...
   readShaderSource("bounce.vert", &VertexShaderSource );
...
//printf("%f\n",(float) *pf );
   glPointSize ( 8 );
   float h = 12.0;				//initial height
   glBegin ( GL_POINTS );
     glVertexAttrib3f ( velParam, 0.0f, 0.0f, 0.0f );	//send vel to vertex shader, assume initial vel = 0 
     glVertex2f ( 0.0f, h );			//starting position of bounce
    // glVertex2f ( -10, 0 );			//starting position of bounce
     //glVertexAttrib3f ( velParam, 5, 10, 0 );	//send vel to vertex shader 
     //glVertexAttrib3f ( velParam, 0, -10, 0 );	//send vel to vertex shader 
     //glVertex2f ( -10, -2 );			//starting position of bounce
     //glVertex2f ( 0, -2 );			//starting position of bounce
   glEnd();
   glutSwapBuffers();
   glFlush();
